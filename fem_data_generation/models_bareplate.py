"""FEM problem definition for the bare-plate thermal simulation in metal AM.

Adapted from the thermal application examples in JAX-AM
(https://github.com/CMSL-HKUST/jax-am).
"""
import numpy as onp
import jax
...
class Thermal(FEM):
    """Transient heat conduction with Neumann BCs (laser flux, convection, radiation)."""

    def custom_init(self, old_sol, rho, Cp, dt, external_faces, k):
        self.rho = rho
        self.Cp = Cp
        self.dt = dt
        self.k = k
